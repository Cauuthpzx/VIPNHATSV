...
export async function proxyRoutes(app: FastifyInstance) {
  ...
  app.post("/invite-list", { preHandler: [authorize(PERMISSIONS.INVITE_READ)], ...PROXY_RATE }, inviteListLocalHandler);

  // Báo cáo
  app.post("/report-lottery", { preHandler: [authorize(PERMISSIONS.REPORT_READ)], ...PROXY_RATE }, reportLotteryHandler);
  app.post("/report-funds", { preHandler: [authorize(PERMISSIONS.REPORT_READ)], ...PROXY_RATE }, reportFundsHandler);
  app.post("/report-third-game", { preHandler: [authorize(PERMISSIONS.REPORT_READ)], ...PROXY_RATE }, reportThirdGameHandler);

  // Tài chính
  app.post("/bank-list", { preHandler: [authorize(PERMISSIONS.FINANCE_READ)], ...PROXY_RATE }, bankListHandler);
  app.post("/deposit-list", { preHandler: [authorize(PERMISSIONS.FINANCE_READ)], ...PROXY_RATE }, depositListHandler);
  app.post("/withdrawals-record", { preHandler: [authorize(PERMISSIONS.FINANCE_READ)], ...PROXY_RATE }, withdrawalsRecordHandler);
  ...
  // Hoàn trả
  app.post("/rebate-odds", { preHandler: [authorize(PERMISSIONS.REBATE_READ)], ...PROXY_RATE }, rebateOddsHandler);
  app.post("/lottery-dropdown", { preHandler: [authorize(PERMISSIONS.REBATE_READ)], ...PROXY_RATE }, getLotteryHandler);

  // Mật khẩu (action endpoints — more restrictive)
  app.post("/edit-password", { preHandler: [authorize(PERMISSIONS.PASSWORD_WRITE)], ...ACTION_RATE }, editPasswordHandler);
  app.post("/edit-fund-password", { preHandler: [authorize(PERMISSIONS.PASSWORD_WRITE)], ...ACTION_RATE }, editFundPasswordHandler);
}
