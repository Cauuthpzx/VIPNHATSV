...
const typeOption = computed(() => {
  ...
  return {
    ...
    series: [
      {
        name: t("analyticsMembers.members"),
        type: "bar",
...
const churnOption = computed(() => {
  ...
  return {
    tooltip: { trigger: "axis", formatter: tooltipFormatter },
    legend: {
      data: [t("analyticsMembers.newMembers"), t("analyticsMembers.lostMembers"), t("analyticsMembers.net")],
      bottom: 0,
    },
    ...
    series: [
      {
        name: t("analyticsMembers.newMembers"),
        type: "bar",
        stack: "churn",
        data: ct.map((d) => d.newMembers),
        itemStyle: { color: COLORS.green },
      },
      {
        name: t("analyticsMembers.lostMembers"),
        type: "bar",
        stack: "churn",
        data: ct.map((d) => -d.lostMembers),
        itemStyle: { color: COLORS.red },
      },
      {
        name: t("analyticsMembers.net"),
        type: "line",
        smooth: true,
        data: ct.map((d) => d.net),
        itemStyle: { color: COLORS.blue },
...
const registrationOption = computed(() => {
  ...
  return {
    ...
    series: [
      {
        name: t("analyticsMembers.register"),
        type: "bar",
        data: rm.map((d) => d.count),
    ...
    <template v-else-if="data">
      ...
      <div class="kpi-row">
        <div class="kpi-mini">
          <div class="kpi-mini-label">
            {{ t("analyticsMembers.totalMembers") }}
          </div>
      ...
      <lay-row :space="15">
        ...
        <lay-col :md="12">
          <lay-card class="section-card">
            <div class="section-header">
              {{ t("analyticsMembers.memberStatus") }}
            </div>
      ...
      <lay-card class="section-card">
        <div class="section-header">
          {{ t("analyticsMembers.registerByMonth") }}
        </div>
      ...
      <lay-row :space="15">
        <lay-col :md="12">
          <lay-card class="section-card">
            <div class="section-header">
              {{ t("analyticsMembers.topBalance") }}
            </div>
            <table class="data-table">
              <thead>
                <tr>
                  <th>{{ t("analyticsMembers.rank") }}</th>
                  <th>Username</th>
                  <th>{{ t("analyticsMembers.agent") }}</th>
                  <th style="text-align: right">
                    {{ t("analyticsMembers.balance") }}
                  </th>
                </tr>
              </thead>
              <tbody>
            ...
            <table class="data-table">
              <thead>
                <tr>
                  ...
                  <th>{{ t("analyticsMembers.agent") }}</th>
                  <th style="text-align: right">
                    {{ t("analyticsMembers.totalDeposit") }}
                  </th>
                  <th style="text-align: right">
                    {{ t("analyticsMembers.depositCount") }}
                  </th>
                </tr>
              </thead>
              <tbody>
