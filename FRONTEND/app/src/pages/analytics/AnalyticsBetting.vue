...
const lotteryTrendOption = computed(() => {
  ...
  return {
    ...
    series: [
      {
        name: t("analyticsBetting.bet"),
        type: "bar",
        data: lt.map((d) => d.betAmount),
...
const thirdTrendOption = computed(() => {
  ...
  return {
    ...
    series: [
      {
        name: t("analyticsBetting.betThird"),
        type: "bar",
        data: tt.map((d) => d.betAmount),
...
const profitOption = computed(() => {
  ...
  return {
    ...
    legend: {
      data: [
        t("analyticsBetting.profitLottery"),
        t("analyticsBetting.profitThird"),
        t("analyticsBetting.total"),
      ],
      bottom: 0,
    },
    grid: { top: 10, right: 20, bottom: 55, left: 60, containLabel: false },
    ...
    series: [
      {
        name: t("analyticsBetting.profitLottery"),
        type: "bar",
        stack: "profit",
        data: pd.map((d) => d.lotteryProfit),
        itemStyle: { color: COLORS.blue },
      },
      {
        name: t("analyticsBetting.profitThird"),
        type: "bar",
        stack: "profit",
        data: pd.map((d) => d.thirdGameProfit),
        itemStyle: { color: COLORS.yellow },
      },
      {
        name: t("analyticsBetting.total"),
        type: "line",
        smooth: true,
        data: pd.map((d) => d.totalProfit),
    ...
    series: [
      {
        name: t("analyticsBetting.bet"),
        type: "bar",
        data: items.map((d) => d.betAmount).reverse(),
        itemStyle: { color: COLORS.blue },
      },
    ],
  };
    ...
    <template v-else-if="data">
      ...
      <div class="kpi-row">
        <div class="kpi-mini">
          <div class="kpi-mini-label">
            {{ t("analyticsBetting.totalBetLottery") }}
          </div>
          <div class="kpi-mini-value text-blue">
            {{ formatMoney(totalLotteryBet) }}
          </div>
          <div class="kpi-mini-sub">
            {{ totalBetCount.toLocaleString() }} {{ t("analyticsBetting.orders") }}
          </div>
        </div>
        <div class="kpi-mini">
          <div class="kpi-mini-label">
            {{ t("analyticsBetting.totalBetThird") }}
          </div>
          <div class="kpi-mini-value" style="color: #ffb800">
            {{ formatMoney(totalThirdBet) }}
          </div>
          <div class="kpi-mini-sub">
            {{ data.thirdGameTrend.reduce((s: number, d: any) => s + d.betTimes, 0).toLocaleString() }}
            {{ t("analyticsBetting.plays") }}
          </div>
        </div>
        <div class="kpi-mini">
          <div class="kpi-mini-label">
            {{ t("analyticsBetting.totalProfit") }}
          </div>
          <div class="kpi-mini-value" :class="totalProfit >= 0 ? 'text-green' : 'text-red'">
            {{ formatMoney(totalProfit) }}
          </div>
        </div>
        <div class="kpi-mini">
          <div class="kpi-mini-label">
            {{ t("analyticsBetting.lotteryTypes") }}
          </div>
          <div class="kpi-mini-value">
            {{ data.lotteryByType.length }}
          </div>
          <div class="kpi-mini-sub">
            {{ data.platformRanking.length }} {{ t("analyticsBetting.thirdPlatforms") }}
          </div>
        </div>
      </div>
      ...
      <lay-card class="section-card">
        <div class="section-header">
          {{ t("analyticsBetting.lotteryTrend") }}
        </div>
      ...
      <lay-card class="section-card">
        <div class="section-header">
          {{ t("analyticsBetting.profitByDay") }}
        </div>
      ...
      <lay-row :space="15">
        <lay-col :md="12">
          <lay-card class="section-card">
            <div class="section-header">
              {{ t("analyticsBetting.betByLottery") }}
            </div>
        ...
        <lay-col :md="12">
          <lay-card class="section-card">
            <div class="section-header">
              {{ t("analyticsBetting.platformShare") }}
            </div>
      ...
      <lay-card class="section-card">
        <div class="section-header">
          {{ t("analyticsBetting.platformRanking") }}
        </div>
        <div style="overflow-x: auto">
          <table class="data-table">
            <thead>
              <tr>
                <th>{{ t("analyticsBetting.rank") }}</th>
                <th>{{ t("analyticsBetting.platform") }}</th>
                <th style="text-align: right">
                  {{ t("analyticsBetting.bet") }}
                </th>
                <th style="text-align: right">
                  {{ t("analyticsBetting.revenue") }}
                </th>
                <th style="text-align: right">Win/Lose</th>
                <th style="text-align: right">
                  {{ t("analyticsBetting.plays") }}
                </th>
              </tr>
            ...
            <tbody>
              <tr v-for="(p, i) in data.platformRanking" :key="p.name">
                ...
                <td style="text-align: right">
                  {{ formatMoney(p.turnover) }}
                </td>
                <td style="text-align: right" :class="p.winLose >= 0 ? 'text-red' : 'text-green'">
                  {{ formatMoney(Math.abs(p.winLose)) }}
                </td>
                <td style="text-align: right">
                  {{ p.betTimes.toLocaleString() }}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="(u, i) in data.topBettors" :key="u.username">
              ...
              <td>{{ u.agentName }}</td>
              <td style="text-align: right">
                {{ formatMoney(u.betAmount) }}
              </td>
              <td style="text-align: right" :class="u.winLose >= 0 ? 'text-red' : 'text-green'">
                {{ formatMoney(Math.abs(u.winLose)) }}
              </td>
            </tr>
          </tbody>
        </table>
